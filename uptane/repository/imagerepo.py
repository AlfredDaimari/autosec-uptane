...
def repo(reponame: str, filename: str):
    ...
    try:
        ...
        if flask.request.method == "GET":
            ...
            # {"signed":{"hash":"hash of zip file", "bufsize":int}, "keyid":"KNaCaMgAlZnFePbHCuHgAgAuPt", "signature":"some_signature"}
            ...
            if not uptane.crypto.hash.get_file_hash('public_repo/{}/{}'.format(reponame, filename), \
                hashf=uptane.crypto.hash.HashFunc.sha256, bufsize=auth_recv_dict["signed"]["bufsize"]):
                return "", 401
            
            # --- 
            # unzipping the file
            with zipfile.ZipFile('public_repo/{}/{}'.format(reponame,
                                                            filename)) as zipO:
            ...
            verify_dir = 'public_repo/{}/{}'.format(reponame,
                                                    filename.split('.')[0])
            ...
            os.rename(timestamp_file_path,
                      'public_repo/{}/timestamp.toml'.format(reponame))
            for file in os.listdir(verify_dir):
                os.rename(verify_dir + '/' + file,
                          'public_repo/{}/{}'.format(reponame, file))
